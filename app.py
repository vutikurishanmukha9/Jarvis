...
from src.assistant.workspace_tools import generate_word_document
...
from src.modules.career import calculate_deep_ats_metrics, get_resume_skills_categorized, get_salary_and_role_estimate
from src.modules.outreach import CampaignManager, EmailDispatcher
from src.modules.vision import clear_active_images, register_uploaded_image
...
st.set_page_config(page_title="J.A.R.V.I.S. — Super-Intelligence", layout="wide", initial_sidebar_state="expanded")
...
with st.sidebar:
    ...
    api_provider = st.selectbox(
        "LLM Provider", list(PROVIDERS.keys()), index=0, help="Select your AI reasoning provider"
    )
    ...
    if saved_sessions:
        selected_session = st.selectbox("Resume Past Session", ["Current"] + saved_sessions, index=0)
    ...
    if st.session_state.processed_file_hash != current_hash:
        with st.spinner("Processing and vectorizing documents..."):
            v_store, summaries, status_msg = process_documents_and_build_vector_store(
                doc_files, api_provider=api_provider, chunk_size=chunk_size, chunk_overlap=chunk_overlap
            )
...
if st.session_state.document_summaries or st.session_state.image_summaries:
    with st.container():
        ...
        for doc in st.session_state.document_summaries:
            ...
            extra = (
                f"{doc.get('pages')} pages"
                if "pages" in doc
                else (f"{doc.get('rows')} rows" if "rows" in doc else f"{doc.get('size')} B")
            )
            st.markdown(f"<span class='apple-pill'>{fname} ({extra})</span>", unsafe_allow_html=True)
        for img in st.session_state.image_summaries:
            st.markdown(
                f"<span class='apple-pill'>{img.get('filename')} ({img.get('dimensions')})</span>",
...
with tab_chat:
    ...
    with col_q2:
        if st.button("Analyze Data & Trends", use_container_width=True):
            quick_prompt = (
                "Perform deep statistical analysis on the uploaded dataset and generate key trend plots using Python."
            )
    with col_q3:
                    ...
                    with st.expander("Thought Process & Tool Telemetry", expanded=False):
                        for step in steps:
                            ...
                            if st_type == "tool_start":
                                ...
                                st.code(
                                    step.get("input", ""),
                                    language="python" if "python" in step.get("tool", "") else "text",
                                )
    ...
    user_query = st.chat_input("Instruct JARVIS (math, documents, vision, web research, workspace files)...")
    if quick_prompt:
    ...
    if user_query:
        ...
        with st.chat_message("assistant"):
            with st.spinner("J.A.R.V.I.S. analyzing query and orchestrating tools..."):
                result = jarvis_engine.run(user_input=user_query, chat_history=st.session_state.chat_history[:-1])
                ...
                SessionManager.save_session(
                    st.session_state.session_id, st.session_state.chat_history, persona=persona_choice
                )
...
with tab_mission:
    ...
    st.caption("Assign high-level goals for JARVIS to plan, execute, and deliver autonomously on your behalf.")
    ...
    if start_mission and goal_input.strip():
        ...
        with st.spinner("Decomposing goal into structured subtasks..."):
            planner = GoalPlanner(
                api_provider=api_provider, api_key=api_key, model_name=model_name, base_url=base_url, temperature=0.1
            )
            doc_context = None
        ...
        runner = AutonomousRunner(orchestrator=jarvis_engine, step_callback=update_ui_callback)
...
with tab_career:
    ...
    st.caption(
        "AI-powered resume optimization, ATS compatibility scoring, skill extraction, and market salary projection."
    )
    ...
    if run_ats_btn:
        if not input_resume_text.strip():
            ...
            with st.spinner("Analyzing resume against ATS engine & ML models..."):
                ...
                ats_results = calculate_deep_ats_metrics(
                    input_resume_text, input_jd_text.strip() if input_jd_text.strip() else input_resume_text
                )
    ...
    if generate_opt_btn and input_resume_text.strip():
        with st.spinner("Generating tailored ATS-optimized resume document in workspace..."):
            ...
            res_path = generate_word_document.invoke(
                {"filename": "optimized_resume.docx", "title": opt_title, "markdown_content": doc_content}
            )
...
with tab_outreach:
    ...
    st.caption(
        "AI-powered personalized outreach, spreadsheet recipient parsing, dynamic tag substitution, and multi-stage follow-up sequences."
    )

    outreach_mode = st.radio(
        "Outreach Workflow Mode",
        ["Campaign Dispatcher & Live Preview", "Multi-Stage Follow-Up Sequence Generator"],
        horizontal=True,
    )
    ...
    if outreach_mode == "Campaign Dispatcher & Live Preview":
        ...
        with col_o2:
            ...
            template_names = (
                [v.get("name") for v in template_lib.values()]
                if template_lib
                else [
                    "Recruiter & Hiring Manager Direct Pitch",
                    "HR Recruiter to Candidate Sourcing",
        ...
        if parsed_records:
            ...
            rend_subj = CampaignManager.render_template(
                subj_input, curr_rec, {"candidateName": user_name, "portfolioUrl": "https://linkedin.com"}
            )
            rend_body = CampaignManager.render_template(
                body_input, curr_rec, {"candidateName": user_name, "portfolioUrl": "https://linkedin.com"}
            )
        ...
        with col_dc1:
            is_simulated = st.checkbox(
                "Safe Simulation Mode (Dry-run with preview validation & Excel audit logs, no live emails sent)",
                value=True,
            )
        with col_dc2:
        ...
        st.caption(
            "Generate a 4-stage cadence (Initial Pitch, Day 4 Value Add, Day 8 Soft Nudge, Day 14 Graceful Breakup)."
        )
        ...
        with col_sq2:
            k_skills = st.text_input(
                "Key Technical Skills / Stack:", value="Python, PyTorch, Distributed Systems, Kubernetes"
            )
            k_achieve = st.text_input(
                "Key Quantified Achievement:", value="increased core system throughput by 45% and cut latency by 30ms"
            )
...
with tab_workspace:
    ...
    if not actual_files:
        st.info(
            "The workspace is currently empty. Ask JARVIS to generate reports, spreadsheets, or code to see them here."
        )
    else:
                    ...
                    if f.suffix.lower() in [".txt", ".md", ".py", ".json", ".csv"]:
                        with open(f, "r", encoding="utf-8", errors="ignore") as pf:
                            st.code(
                                ...
                                language="python"
                                if f.suffix == ".py"
                                else ("json" if f.suffix == ".json" else "markdown"),
                            )
...
with tab_profile:
    ...
    with st.form("profile_form"):
        ...
        with col_p2:
            r_desc = st.text_input(
                "Your Role / Focus Area", value=current_prof.get("role_description", "Lead Engineer & Creator")
            )
            p_style = st.text_input(
                "Preferred Output Style",
                value=current_prof.get("preferred_style", "Concise, structured, executive-ready"),
            )
    ...
    if memories:
        for m in memories:
            st.markdown(
                f"- **{m.get('fact')}** (_{m.get('timestamp')}_) <span class='apple-badge blue'>{m.get('category', 'general')}</span>",
                unsafe_allow_html=True,
            )
    else:
    ...
    with col_m1:
        new_fact = st.text_input(
            "Add a persistent memory fact about you or your projects:",
            placeholder="e.g. My primary coding language is Python 3.12 and I prefer dark mode visuals.",
        )
    with col_m2:
