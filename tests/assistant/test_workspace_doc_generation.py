...
def test_write_and_read_workspace_file(tmp_path: Path) -> None:
    ...
    res_write = write_workspace_file.invoke({"filename": "test_report.md", "content": "# Test Heading\nThis is test content."})
...
def test_generate_excel_spreadsheet_from_json() -> None:
    ...
    res = generate_excel_spreadsheet.invoke({"filename": "finance.xlsx", "json_table_data": json_data, "sheet_name": "Q1_Q2_Revenue"})
...
def test_generate_word_document_from_markdown() -> None:
    ...
    md_content = "# Title\n## Section 1\n- Item 1\n- Item 2\n1. Numbered Item"
    res = generate_word_document.invoke({"filename": "summary.docx", "title": "Summary Report", "markdown_content": md_content})
