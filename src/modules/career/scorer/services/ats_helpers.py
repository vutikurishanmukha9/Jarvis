...
def extract_experience_duration(text: str) -> Dict[str, Any]:
    ...
    """
    from src.modules.career.scorer.utils.feature_extractor import extract_years_of_experience

    total_years = extract_years_of_experience(text)

    result = {"total_years": total_years, "skill_years": {}, "date_ranges": []}
...
def detect_education_level(text: str) -> Dict[str, Any]:
    ...
    result = {"highest_level": "unknown", "level_score": 0, "degrees_found": []}
...
def detect_seniority_level(text: str) -> Dict[str, Any]:
    ...
    result = {"level": "unknown", "level_score": 0, "indicators_found": []}
        ...
        for header in headers:
            ...
            for match in matches:
                header_positions.append(
                    {"type": section_type, "header": header, "start": match.end(), "header_start": match.start()}
                )
...
def extract_achievements(text: str) -> List[Dict[str, Any]]:
    ...
    for sentence in sentences:
        ...
        metrics = {
            ...
            "time": bool(re.search(r"\d+\s*(?:hours?|days?|weeks?|months?|x\s*faster|x\s*improvement)", clause)),
        ...
        if has_metric:
            achievements.append({"text": sentence, "metrics": [k for k, v in metrics.items() if v]})
...
def classify_jd_keywords(jd_text: str, all_keywords: List[str]) -> Dict[str, List[str]]:
    ...
    result = {"required": [], "preferred": [], "standard": []}
...
def match_job_title(resume_text: str, jd_title: str) -> Dict[str, Any]:
    ...
    result = {"score": 0, "matched_category": None, "resume_titles": [], "jd_category": None}
...
def detect_keyword_stuffing(resume_text: str, keywords: List[str]) -> Dict[str, Any]:
    ...
    """
    result = {"is_stuffed": False, "penalty": 0, "flagged_keywords": [], "keyword_density": 0.0}
...
def calculate_recency_bonus(text: str) -> Dict[str, Any]:
    ...
    result = {"bonus": 0, "most_recent_year": None, "gap_years": 0, "details": ""}
...
def get_score_interpretation(score: int) -> Dict[str, str]:
    ...
    if score >= 85:
        ...
        return {"badge": "Good", "color": "blue", "message": "Worth applying - Minor gaps to address"}
    elif score >= 50:
        return {"badge": "Fair", "color": "yellow", "message": "Needs improvement - Notable gaps in requirements"}
    else:
        return {
            "badge": "Poor",
            "color": "red",
