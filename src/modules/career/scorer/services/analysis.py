...
from src.modules.career.scorer.config import MIN_TEXT_LENGTH, TOP_MATCHES, MATCHING_WEIGHTS, MAX_TEXT_LENGTH
from src.modules.career.scorer.services.model_manager import model_manager
from src.modules.career.scorer.exceptions import AnalysisError, TextExtractionError
...
def analyze_resume(resume_text: str) -> Tuple[str, List[Tuple[str, float]], float, Dict[str, Any]]:
    ...
    try:
        ...
        try:
            ...
            base_salary = float(600000 + (features['years_experience'] * 250000) + (features['seniority_level'] * 200000) + (features['education_level'] * 150000) + (min(features['skills_count'], 20) * 25000))
