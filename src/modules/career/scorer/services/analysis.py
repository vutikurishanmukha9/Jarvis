...
from src.modules.career.scorer.exceptions import AnalysisError
...
def analyze_resume(resume_text: str) -> Tuple[str, List[Tuple[str, float]], float, Dict[str, Any]]:
    ...
    try:
        ...
        resume_embed = model_manager.embed_model.encode([resume_text], convert_to_tensor=True)
        ...
        try:
            ...
            base_salary = float(
                600000
                + (features["years_experience"] * 250000)
                + (features["seniority_level"] * 200000)
                + (features["education_level"] * 150000)
                + (min(features["skills_count"], 20) * 25000)
            )
        ...
        raise AnalysisError(f"Failed to analyze resume: {str(e)}") from e
...
def calculate_jd_resume_match(resume_text: str, jd_text: str) -> Tuple[float, Dict[str, Any]]:
    ...
    try:
        ...
        jd_embedding = model_manager.embed_model.encode(jd_text, convert_to_tensor=True)
        ...
        resume_sentences = [s for s in split_into_sentences(resume_text) if _is_substantive_sentence(s)][:12]
        jd_sentences = [s for s in split_into_sentences(jd_text) if _is_substantive_sentence(s)][:12]

        if resume_sentences and jd_sentences:
            resume_sent_embeds = model_manager.embed_model.encode(resume_sentences, convert_to_tensor=True)
            jd_sent_embeds = model_manager.embed_model.encode(jd_sentences, convert_to_tensor=True)
        ...
        raise AnalysisError(f"Failed to calculate match: {str(e)}") from e
    except Exception as e:
