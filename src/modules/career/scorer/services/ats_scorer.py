...
from src.modules.career.scorer.services.ats_constants import (
...
def generate_suggestions(
    ...
    if critical_missing:
        if len(critical_missing) >= 5:
            ...
            suggestions.append(f"Add these required keywords: {', '.join(critical_missing)}")
        else:
            suggestions.append(f"Add the required keyword '{critical_missing[0]}' to match the job description")
    ...
    if important_missing and len(suggestions) < 4:
        ...
        suggestions.append(f"Consider adding preferred skills: {', '.join(top_important)}")
    ...
    if achievement_count == 0:
        suggestions.append("Add quantified achievements with metrics (%, $, numbers) to show impact")
    ...
    if total_years == 0:
        suggestions.append("Specify your total years of experience clearly (e.g., '5+ years of experience')")
    elif not skill_years:
        suggestions.append("Add years of experience for key skills (e.g., 'Python - 4 years', 'AWS - 2 years')")
    ...
    if title_score is not None and title_score < 50:
        suggestions.append("Include the target job title or related titles in your resume summary")

    # Priority 8: Education suggestions
    if education_score is not None and education_score < 40:
        suggestions.append("Clearly list your educational qualifications with degree names")
    ...
    for detail in formatting_penalty.get("details", []):
        if "bullet" in detail.lower() and len(suggestions) < 6:
            suggestions.append("Use more bullet points (aim for 3-5 per job) to highlight key accomplishments")
        elif "contact" in detail.lower() and len(suggestions) < 6:
            suggestions.append("Add complete contact information: email, phone, LinkedIn profile")
        elif "date" in detail.lower() and len(suggestions) < 6:
            suggestions.append("Use specific date ranges (e.g., 'Jan 2020 - Dec 2023') instead of vague dates")
    ...
    if len(suggestions) < 3:
        ...
        if optional_missing and len(optional_missing) > 3:
            suggestions.append(f"Bonus: Include optional skills like {', '.join(optional_missing[:2])} if applicable")
...
class ATSScorer:
    ...
    def calculate_skill_score(
        ...
        matched = {"required": [], "preferred": [], "standard": []}
        ...
        if field_weights_max > 0:
            ...
            total = (
                len(jd_keywords.get("required", []))
                + len(jd_keywords.get("preferred", []))
                + len(jd_keywords.get("standard", []))
            )
        ...
        if self.mode == "deep" and resume_skills:
            frequency_bonus = 0.0
            all_matched = matched["required"] + matched["preferred"] + matched["standard"]
            for kw in all_matched:
                count = resume_skills_lower.count(kw.lower())
                if count < 2 and sections:
                    section_hits = sum(1 for sec in sections.values() if kw.lower() in sec.lower())
        ...
        return {"score": int(base_score), "matched": matched, "missing": missing}
    ...
    def calculate_title_score(self, resume_text: str, jd_title: str) -> Dict[str, Any]:
        ...
        return {"score": min(100, base_score + seniority_bonus), "title_match": title_match, "seniority": seniority}

    def calculate_experience_score(self, resume_experience: Dict, required_years: int = 0) -> Dict[str, Any]:
        ...
        return {"score": score, "total_years": total_years, "skill_years": resume_experience.get("skill_years", {})}
    ...
    def calculate_achievement_score(self, achievements: List[Dict]) -> Dict[str, Any]:
        ...
        return {"score": score, "count": count, "achievements": achievements[:5]}

    def calculate_education_score(self, resume_education: Dict, required_level: str = None) -> Dict[str, Any]:
        ...
        return {"score": max(score, 20), "level": level, "degrees_found": resume_education.get("degrees_found", [])}
    ...
    def _quick_scan(
        self, resume_text: str, jd_text: str, resume_skills: List[str] = None, jd_keywords: List[str] = None
    ) -> Dict[str, Any]:
    ...
    def _deep_analysis(
        ...
        skill_result = self.calculate_skill_score(
            resume_skills or [], classified_keywords, semantic_similarity, sections=sections
        )
        ...
        return {
            ...
            "education_details": {"level": education_result["level"], "degrees": education_result["degrees_found"]},
            "sections_detected": list(sections.keys()),
            "stuffing_detected": stuffing["is_stuffed"],
            "required_gate_applied": (total_required > 0 and missing_required > (total_required / 2)),
            "suggestions": suggestions,
        }


# Convenience function for external use
def calculate_ats_score(resume_text: str, jd_text: str, mode: str = "deep", **kwargs) -> Dict[str, Any]:
    """
