...
from src.modules.career.scorer.config import MAX_JD_LENGTH
from src.modules.career.scorer.services.analysis import calculate_jd_resume_match
...
async def match_jd_resume(request: Request, resume: UploadFile = File(...), jd_text: str = Form(...)):
    ...
    try:
        ...
        match_percentage, detailed_results = await run_in_threadpool(calculate_jd_resume_match, resume_text, jd_text)

        track_analysis(
            "jd_match",
            {"match_percentage": match_percentage, "component_scores": detailed_results.get("component_scores", {})},
        )
        ...
        raise HTTPException(status_code=500, detail="Failed to calculate match percentage. Please try again.") from e
