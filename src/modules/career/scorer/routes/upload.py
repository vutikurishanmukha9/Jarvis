...
from src.modules.career.scorer.config import CURRENCY_SYMBOL
...
async def upload_resume(request: Request, resume: UploadFile = File(...)):
    ...
    try:
        ...
        predicted_job, matches, salary, salary_details = await run_in_threadpool(analyze_resume, resume_text)

        track_analysis(
            "upload",
            {"predicted_job": predicted_job, "salary": int(salary), "confidence": salary_details["confidence"]},
        )
