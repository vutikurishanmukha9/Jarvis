...
class LocalBlipService(LLMService):
    ...
    def answer_question_batch(
        self, images: List[Image.Image], questions: List[str], contexts: Optional[List[str]] = None
    ) -> List[str]:
        try:
...
class LlaVAService(LLMService):
    ...
    def __init__(
        ...
        self.model_id = model_id
        self.model_revision = model_revision or os.getenv("JARVIS_LLAVA_MODEL_REVISION", "main")
        if device == "auto":
            self.device = "cuda" if torch.cuda.is_available() else "cpu"
        else:
            self.device = device
    ...
    def answer_question(self, image: Image.Image, question: str, context: str = "") -> str:
        ...
        try:
            ...
            # Generate
            generate_ids = self.model.generate(**inputs, max_new_tokens=200, do_sample=True, temperature=0.7, top_p=0.9)
    ...
    def answer_question_batch(
        ...
        try:
            ...
            prompts = []
            for q, c in zip(questions, contexts, strict=True):
                full_prompt = "USER: <image>\n"
                if c:
            ...
            inputs = self.processor(text=prompts, images=images, padding=True, return_tensors="pt").to(
                self.model.device
            )
