...
class BaseFactory(Generic[A], metaclass=ABCMeta):
    ...
    def load_from_plugins(
        ...
        if not self._classes:
            try:
                import docling.models.plugins.defaults as docling_defaults

                attr = getattr(docling_defaults, self.plugin_attr_name, None)
                if callable(attr):
                    config = attr()
                    self.process_plugin(
                        config, "docling_defaults", "docling.models.plugins.defaults"
                    )
            except Exception as e:
                logger.debug("Failed to fallback register defaults: %s", e)
